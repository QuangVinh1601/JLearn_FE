...
const CourseDetail = () => {
  ...
  if (isLoading && !course) {
    // Hiển thị loading chỉ khi chưa có dữ liệu course
  ...
  return (
    <div className="course-detail-container">
      ...
      {/* Nút Mua khóa học */}
      {isPurchased ? (
        <button disabled className="purchase-button purchased">
          Đã mua
        </button>
      ) : (
        <button
      ...
      {userBalance < course.price && !isPurchased && (
        <p style={{ color: "orange" }}>Không đủ số dư</p>
      )}
