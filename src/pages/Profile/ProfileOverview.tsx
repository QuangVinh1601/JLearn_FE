...
const ProfileOverview: React.FC = () => {
  ...
  const { userInfo, loading, error, refreshUserInfo, clearCache, userID } =
    useUserInfo();
        ...
        >
          <div className="text-center py-8">
            <p className="text-xl text-red-600">
              Lỗi tải thông tin người dùng: {error}
            </p>
          ...
          <div className="flex flex-col lg:flex-row items-center lg:items-start space-y-6 lg:space-y-0 lg:space-x-8">
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="flex flex-wrap gap-3 justify-center lg:justify-start">
                <span
                  className="flex items-center px-4 py-2 text-red-800 rounded-full text-sm font-medium shadow-sm hover:shadow-md transition-shadow"
                  style={{ backgroundColor: "white" }}
                >
                ...
                <span className="flex items-center px-4 py-2 bg-green-100 text-green-800 rounded-full text-sm font-medium shadow-sm hover:shadow-md transition-shadow">
                  {FaGraduationCap({ className: "mr-2" })} {courseCount} khóa
                  học
                </span>
                <span
                  className="flex items-center px-4 py-2 text-red-800 rounded-full text-sm font-medium shadow-sm hover:shadow-md transition-shadow"
                  style={{ backgroundColor: "white" }}
                >
                  {FaLayerGroup({ className: "mr-2" })} {flashcardCount} bộ
                  flashcard
                </span>
                {userInfo.createdAt && (
                  <span className="flex items-center px-4 py-2 bg-yellow-100 text-yellow-800 rounded-full text-sm font-medium shadow-sm hover:shadow-md transition-shadow">
                    {FaCalendarAlt({ className: "mr-2" })} Tham gia{" "}
                    {formatCreatedDate(userInfo.createdAt)}
                  </span>
                )}
              </div>
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            {[
              ...
              <div
                key={item.label}
                className="rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow duration-300 border"
                style={{ backgroundColor: "white", borderColor: "#D4B896" }}
              >
                ...
                <p className="text-lg text-gray-800 ml-8 font-medium">
          ...
          {[
            {
              ...
              description: "Xem các khóa học đã mua",
