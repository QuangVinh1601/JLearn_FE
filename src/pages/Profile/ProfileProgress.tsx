...
import { FaTrophy, FaFire, FaCalendarAlt, FaChartLine, FaStar, FaGraduationCap } from 'react-icons/fa';
...
const ProfileProgress: React.FC = () => {
    ...
    const achievements = [
        { id: 1, title: "Học viên mới", description: "Hoàn thành bài học đầu tiên", icon: "🎯", unlocked: true },
        { id: 2, title: "Kiên trì", description: "Học liên tục 7 ngày", icon: "🔥", unlocked: true },
        { id: 3, title: "Từ vựng thành thạo", description: "Học 100 từ vựng", icon: "📚", unlocked: true },
        { id: 4, title: "Chuyên gia ngữ pháp", description: "Hoàn thành 50 bài tập ngữ pháp", icon: "⚡", unlocked: false },
        { id: 5, title: "Bậc thầy", description: "Đạt cấp độ N2", icon: "👑", unlocked: false },
        { id: 6, title: "Siêu học viên", description: "Học liên tục 30 ngày", icon: "🌟", unlocked: false }
    ];
    ...
    const completedDays = weeklyActivity.filter(day => day.hours >= day.target).length;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="rounded-3xl shadow-xl p-6 md:p-8" style={{ backgroundColor: '#F5E6CA' }}>
                ...
                <p className="text-gray-600">Theo dõi quá trình học tập và thành tích của bạn</p>
            </div>
            ...
            {/* Level Progress */}
            <div className="rounded-3xl shadow-xl p-6 md:p-8" style={{ backgroundColor: '#F5E6CA' }}>
                <h2 className="text-2xl font-bold text-gray-800 mb-6">Tiến độ cấp độ</h2>
                <div className="rounded-2xl p-6 border" style={{
                    ...
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-lg font-bold text-red-600">{progressData.levelProgress}%</span>
                    </div>
                    ...
                    <p className="text-sm text-gray-600">
                        Bạn đã hoàn thành {progressData.levelProgress}% để đạt cấp độ {progressData.nextLevel}.
                        Hãy tiếp tục học tập để tiến bộ hơn nữa!
                    </p>
                </div>
            </div>
            ...
            <div className="rounded-3xl shadow-xl p-6 md:p-8" style={{ backgroundColor: '#F5E6CA' }}>
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-gray-800">Hoạt động tuần này</h2>
                    <div className="text-right">
                        <div className="text-2xl font-bold text-green-600">{progressData.weeklyProgress}h</div>
                        <div className="text-sm text-gray-500">/ {progressData.weeklyGoal}h mục tiêu</div>
                    </div>
                </div>
                    ...
                    {weeklyActivity.map((day, index) => (
                        <div key={index} className="text-center">
                            ...
                            <div className="relative">
                                <div className="w-full h-20 rounded-lg flex items-end p-1" style={{ backgroundColor: '#F0D5A8' }}>
                                    <div
                                        className={`w-full rounded transition-all duration-500 ${day.hours >= day.target ? 'bg-green-500' : day.hours > 0 ? 'bg-yellow-500' : 'bg-gray-300'
                                            }`}
                                        style={{ height: `${Math.max((day.hours / day.target) * 100, 10)}%` }}
                                    ></div>
                                </div>
            ...
            {/* Achievements */}
            <div className="rounded-3xl shadow-xl p-6 md:p-8" style={{ backgroundColor: '#F5E6CA' }}>
                <h2 className="text-2xl font-bold text-gray-800 mb-6">Thành tích đạt được</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {achievements.map((achievement) => (
                        <div
                            key={achievement.id}
                            className={`p-6 rounded-2xl border-2 transition-all duration-300 ${achievement.unlocked
                        ...
                        >
                            <div className="text-center">
                                ...
                                <h3 className={`font-bold text-lg mb-2 ${achievement.unlocked ? 'text-gray-800' : 'text-gray-400'
                                    }`}>
                                    {achievement.title}
                                </h3>
